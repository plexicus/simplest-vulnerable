...
if(isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM usuarios WHERE id = ?"); // Usando prepared statements para evitar SQL Injection
    $stmt->bind_param("i", $id); // Vinculando el parámetro 'id' como un entero
    $stmt->execute();
