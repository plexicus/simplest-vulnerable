...
if(isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            echo "id: " . htmlspecialchars($row["id"]). " - Nombre: " . htmlspecialchars($row["nombre"]). "<br>"; // Uso de htmlspecialchars para prevenir XSS
        }
    } else {
        echo "0 resultados";
    }
...
if(isset($_GET['mensaje'])) {
    $mensaje = htmlspecialchars($_GET['mensaje']); // Uso de htmlspecialchars para prevenir XSS
